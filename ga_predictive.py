...
def new_report_data(report_data_list=None):
    # env_data = db.reportdatas
    # data = env_data.find()
    # data1 = list(data)
    results = pd.json_normalize(report_data_list)
...
def carbon_new_data(report_data_list=None):
    ...
    df1 = df1[
        ['companyId', 'category', 'factorName', 'factorName1', 'locationId', 'scope', 'subCategory1', 'subCategory2',
         'subCategory3', 'subCategory4', 'subCategory5', 'subCategory6', 'subCategory7', 'subCategory8', 'subCategory9',
         'emission_factor', 'fuel_quantity', 'year', 'month', 'units', 'value']]
    cols = ["year", "month"]
...
@app.route('/report_data_prdiction_autoreg/<year>', methods=['GET', 'POST'])
def report_data_prdiction(year):
    if request.method == 'POST':
        companyId = request.form.get("companyId", None)
        # print("====u_email", companyId)
        mongo_string = request.form.get("mongo_string", None)
        client = MongoClient(str(mongo_string))
        db = client['greenalytics-testing-db']
        env_data = db.reportdatas
        data = env_data.find({"companyId":str(companyId)})
        report_data_list = list(data)
        df = new_report_data(report_data_list)
        print("======>report_data", df)
        years = []
        ...
        for data, value in df_new.iterrows():
            ...
            for index, ele in enumerate(list1):
                if ele['category'] == value['category'] and ele['locationId'] == value['locationId'] and ele[
                    'companyId'] == value['companyId'] and ele['factorName'] == value['factorName'] and ele[
                    'factorName1'] == value['factorName1'] and ele['subCategory1'] == \
                        value['subCategory1'] and ele['subCategory2'] == value['subCategory2'] and ele[
                    'subCategory3'] == \
                        value['subCategory3'] and ele['subCategory4'] == value['subCategory4'] and ele[
                    'subCategory5'] == \
                        value['subCategory5'] and ele['subCategory6'] == value['subCategory6'] and ele[
                    'subCategory7'] == \
                        value['subCategory7'] and ele['subCategory8'] == value['subCategory8'] and ele[
                    'subCategory9'] == \
                        value['subCategory9'] and ele['emission_factor'] == value['emission_factor'] and ele[
                    'fuel_quantity'] == \
        ...
        return Response({"email":companyId  ,"password": mongo_string})
    return Response("200 ok")


@app.route('/carbon_data_pred_autoreg/<year>', methods=['GET', "POST"])
def carbon_data_pred_autoreg(year):
    if request.method == "POST":
        companyId = request.form.get("companyId", None)
        # print("====u_email", companyId)
        mongo_string = request.form.get("mongo_string", None)
        ...
        env_data = db.carbondatas
        data = env_data.find({"companyId": str(companyId)})
        carbon_data_list = list(data)
        df = carbon_new_data(carbon_data_list)
        years = []
        # df = carbon_data()
        for i in list(df.columns[5:]):
            years.append(i[-4:])
        ...
        # print("___result", result)
        for val in list1:
            update_database = db.test_predict_carbon.update_one({'companyId': val['companyId'],
                                                                'locationId': val['locationId'],
                                                                'factorName': val['factorName'],
                                                                'factorName1': val['factorName1'],
                                                                'category': val['category'],
